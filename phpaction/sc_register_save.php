...
if (isset($_POST['register'])){
    ...
    $allow = array('.jpg', '.jpeg', '.gif', '.png'); // only allowed extention for changing profile pict

    $allow_mc = array('.pdf'); // only allowed extention for changing profile pict

    $today = date("Y-m-d");
    $diff = date_diff(date_create($dob), date_create($today));
    $real_age = $diff->format('%y');

    if($age != $real_age){
        ?>
        <script>
            alert('Your age from date of birth is not qualified for Senior Citizens ID and not equal to registered age');
            window.location.href = "../register_sc.php";
        </script>
        <?php
    }else{
        if(mysqli_num_rows($pwd_result)>0 || mysqli_num_rows($sc_result)>0 || mysqli_num_rows($admin_result)>0){
            ...
            else
            {
                ...
                if ($conn->query($query) === TRUE) {
                    ...
                    $Correo->Subject = "SENIOR CITIZEN'S IDENTIFICATION CARD APPLICATION REQUEST COMPLETED";
                    $Correo->Body = "<P>Hi good day!</P>
                                    ...
                                    <P>Access Type: <B>SENIOR CITIZEN</B></P>
